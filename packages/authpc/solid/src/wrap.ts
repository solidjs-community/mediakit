...
import { AuthPClientError } from './error'
...
export async function wrapFn<Fn extends ExpectedFn$<any, any>>(
  ...
  try {
    ...
    if (response instanceof Response) {
      ...
      if (response.headers.get('X-AuthPC-Error') === '1') {
        const isValidationError =
          response.headers.get('X-AuthPC-Validation') === '1'
        const error = await optionalData(response)
        throw new AuthPClientError(
          error.error.message,
    ...
    if (e instanceof AuthPClientError) throw e
    throw new AuthPClientError(e.message, e)
  }
}
