...
- canBeUnmuted - This is a signal that will check if the user interacts or has interacted with the page, if he did, you can start playing audio files.
...
The `Render` fn also takes in an `onFailed` option, this can be used to override the `onFailed` option passed to `createVideo`.
